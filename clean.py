...
# def read_table(table_name, file_extension = ".csv"):
#     """returns the dataframe for the monthly data"""
#     # columns to be parsed as dates
#     columns_as_dates = ['checkin', 'checkout', 'date']
#     # reading the csv file to a pandas dataframe
#     table_df = pd.read_csv(path + table_name + file_extension, parse_dates=columns_as_dates)
#     table_df['city'] = map_city(table_df['hotel_id'])

def return_pivot(table_name, file_path, file_extension = ".csv", status_value = 2):
    """Returns:
...
# reading the excel file containing hotel_id-city mapping
tableD = pd.read_excel(path+"TableD.xlsx")
# creating a dictionary with hotel_id as keys, and city as values
city_mapping = dict(zip(tableD['hotel_id'], tableD['city']))
